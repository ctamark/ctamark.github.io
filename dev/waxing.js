class waxing {
	  ...
	  this.closeup_img.src = "closeup_waxing-01.png";	  
	  //canvas 기준
      ...
      this.stuff_img.src = "waxing_sprite-01.png";
	  ...
	  this.lipbad_img.src = "./lip_bad-01.png"
	   ...
	   this.sound_treatment = new Audio("waxing_effect-01.wav"); 
	 ...
	 // this.spriteArray = [];
	  ...
	  this.totalDeltaMove = 0
	  	  
	  this.isInRangeLastTouch = false

      this.isWait_playstate_rest = false
   ...
   //===============	
	...
	updateFrame(curT){
      ...
      this.appCurT = curT		
	   ...
	   if(gPlaystate == playstate_treatment){
			...
			this.updateWaxingTreatment(curT)
	...
	draw(ctx){
	    ...
	    if(gPlaystate == playstate_treatment || gPlaystate == playstate_wait_rest){
  ...
  drawFace(ctx, faceProp){
   ...
   drawHairStuff(ctx){
	  ...
	  for(let i=0; i < this.stuffArray.length; i++){
				   ...
				   else  if(obj.state == Stuff.state_treated){			   
				   //waxing 끝난 경우 
				   ...
				   let sy = 50*yIdx 			   
				   let cx = this.closeupRscPos.x + obj.drawPosX   
				   let cy = this.closeupRscPos.y + obj.drawPosY
				   //-------
				  // obj.drawPosX = dx
				//   obj.drawPosY = dy 
				   //-------

				   ctx.drawImage(this.stuff_img, sx, sy, 50, 50, cx, cy, 50, 50);														   
	...
	*/   
	...
	for(let i =0 ; i < this.stuffArray.length; i++){
 		  ...
 		  this.isInRangeLastTouch  = false 
			...
			if(dist <= obj.radius){			
				console.log("inRange: " + dist + ", touchX: "+tx + "objCanvasX: " + objCanvasX)						  
				
				obj.state = Stuff.state_treating 

 			   //범위내
				 this.isInRangeLastTouch  = true
 	   ...
 	   this.lastMoveCheckPos = {x:tx, y:ty}
	   this.lastMovePos = {x:tx, y:ty}
	...
	let diffX = tx - this.lastMovePos.x 
	let diffY = ty - this.lastMovePos.y 
	
	let distToLastMove = Math.sqrt(diffX*diffX + diffY*diffY)
				...
				let dist = Math.sqrt(diffX*diffX + diffY*diffY)
             		...
             		if(dist <= obj.radius){						
					  ...
					  //범위내
					   ...
					   this.totalDeltaMove += distToLastMove
					   //let sprite = this.spriteArray[i]
					   //sprite.play()				  			 
					}else {
						
						 if(this.isInRangeLastTouch==true){
				   ...
				   console.log('distToLastMove:' + distToLastMove)
				   console.log('totalDeltaMove:' + this.totalDeltaMove)
			   
			         if(this.totalDeltaMove >= 300){
						 
		 				    obj.state = Stuff.state_treated
							
         				    console.log('obj--->state_treated:')
			      ...
			      //지난 	
                 console.log("obj.userVal: " + obj.userVal)							
	...
	createHair(num){
	   ...
	   //(54, 200)
		...
		//closeup imgSize: 400x400
		 ...
		 //closeUp LT기준 
		 let posArray = [] 
		 posArray.push({x: 130, y: 250}) 		 
		 
		 posArray.push({x: 160, y: 220}) 
	...
	//	 posArray.push({x: 230, y: 230})  
		 ...
		 for(let i=0; i < num; i++){
		   ...
		   let hair = new Stuff();
		   hair.state = Stuff.state_normal 
		   
		   hair.radius = 22;

		   let pos = posArray[i]
		   hair.drawPosX = pos.x*xRatio 
		   hair.drawPosY = pos.y*yRatio 
		   
		   //부모LT기준 hair의 중심점 
		   hair.x = hair.drawPosX + hair.radius
 		   hair.y = hair.drawPosY + hair.radius
		   
		   console.log("hairDrawPos: " + hair.drawPosX + ", " + hair.drawPosY)
						 
		  this.stuffArray.push(hair)
	...
	/*
	  for(let i=0; i < this.spriteArray.length ; i++){
		...
		*/
		...
		updateWaxingPlayState(progress){
...
updateWaxingTreatment(curT){
  ...
  this.updateWaxingPlayState(percent)
 ...
 probeFinishJob(curT){
   ...
   if(gPlaystate != playstate_treatment){
	...
	if(numTreated >= this.numTotalStuff){
			 ...
			 gPlaystate = playstate_wait_rest		
			//너무 빨리 rest로 넘어가면 안됨 
